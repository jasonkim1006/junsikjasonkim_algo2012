...
void vectorField::setupField(int innerW, int innerH, int outerW, int outerH){
    ...
    // total number of "pixels", ie w * h
	for (int i = 0; i < fieldSize; i++){
		ofVec2f pt;
		pt.set(0,0);
        //push points throughout the fieldSize
		field.push_back(pt);
	}
}
...
void vectorField::draw(){
    ...
    for (int i = 0; i < fieldWidth; i++){ //x axis
        for (int j = 0; j < fieldHeight; j++){ //y axis
            ...
            //            line.set(px, py); (0,0) so it will create from (0,0)
			float length = line.length();
			line.normalize();
			line.rotate(90);  // these are angles in degrees
            //JASON draws the PERPENDICULAR line
            ofLine(px - line.x*length*0.2, py - line.y*length*0.2, px + line.x*length*0.2, py + line.y*length*0.2);
//            ofLine(px - line.x*length*0.6, py - line.y*length*0.6, px + line.x*length*0.6, py + line.y*length*0.6);
...
void vectorField::addInwardCircle(float x, float y, float radius, float strength){
    ...
    //JASON changing pctx, pcty, radiusPct (external) into fieldPosX, fieldPosY, fieldRadius (internal)
    ...
    //JASON so that we dont have to check all the pixels (including external).
    //We can just get points within the fieldRadius.
    ...
    for (int i = startx; i < endx; i++){
        for (int j = starty; j < endy; j++){
			...
			if (distance < fieldRadius){
                ...
                //JASON
                //(unit_px, unit_py) is the unit vector direction where the second point of the line is.
                //field[pos].x -= 1 * strongness;
                //field[pos].y -= unit_py * 5;
